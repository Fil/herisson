...
class sfYamlParser
{
  ...
  public function parse($value)
  {
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#S', $this->currentLine, $values))
      {
        if (isset($values['value']) && preg_match('#^&(?P<ref>[^ ]+) *(?P<value>.*)#S', $values['value'], $matches))
        {
          $isRef = $matches['ref'];
        ...
        {
          if (preg_match('/^([^ ]+)\: +({.*?)$/S', $values['value'], $matches))
          {
      ...
      else if (preg_match('#^(?P<key>[^ ].*?) *\:(\s+(?P<value>.+?))?\s*$#S', $this->currentLine, $values))
      {
        ...
        else if (isset($values['value']) && preg_match('#^&(?P<ref>[^ ]+) *(?P<value>.*)#S', $values['value'], $matches))
        {
          $isRef = $matches['ref'];
          $values['value'] = $matches['value'];
        }
  ...
  protected function getNextEmbedBlock()
  {
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^(?P<text> *)$#S', $this->currentLine, $match))
      {
        // empty line
        $data[] = $match['text'];
      }
  ...
  protected function parseValue($value)
  {
    ...
    if (preg_match('/^(?P<separator>\||>)(?P<modifiers>\+|\-|\d+|\+\d+|\-\d+|\d+\+|\d+\-)?(?P<comments> +#.*)?$/S', $value, $matches))
    {
      $modifiers = isset($matches['modifiers']) ? $matches['modifiers'] : '';

      return $this->parseFoldedScalar($matches['separator'], preg_replace('#\d+#S', '', $modifiers), intval(abs($modifiers)));
    }
    else
    {
  ...
  protected function parseFoldedScalar($separator, $indicator = '', $indentation = 0)
  {
    ...
    if (!preg_match('#^(?P<indent>'.($indentation ? str_repeat(' ', $indentation) : ' +').')(?P<text>.*)$#S', $this->currentLine, $matches))
    {
      $this->moveToPreviousLine();
    ...
    while ($this->currentLineNb + 1 < count($this->lines))
    {
      ...
      if (preg_match('#^(?P<indent> {'.strlen($textIndent).',})(?P<text>.+)$#S', $this->currentLine, $matches))
      {
      ...
      else if (preg_match('#^(?P<text> *)$#S', $this->currentLine, $matches))
      {
        $text .= preg_replace('#^ {1,'.strlen($textIndent).'}#S', '', $matches['text'])."\n";
      }
      else
      {
        $this->moveToPreviousLine();
